load("../../Cleaned_Data/Tests/4 Periods/Crypto_ind_periods_4.RData")
load("../../Cleaned_Data/Forecasts/final_data_0.05_2024-05-13_all.RData")
...
test_lhs<-"GRF" #which one do you want to test against; GRF_base or GRF
...
{
  mat_p_val<-matrix(NA,nrow=length(list_crypto_period[[ind_i]]$Crypto_ind),ncol=9) #take QRF, QR, GJR_GARCH, GARCH-X, CAVIAR, Hist,3xbase GRF,QRF,QR, CAV_ASY
  mat_perf<-matrix(NA,nrow=length(list_crypto_period[[ind_i]]$Crypto_ind),ncol=9) #take QRF, QR, GJR_GARCH, GARCH-X, CAVIAR, Hist,3xbase GRF,QRF,QR, CAV_ASY
  ...
  for(ind_k in 1:length(list_crypto_period[[ind_i]]$Crypto_ind))
  {
    ...
    print(paste0("Period: ",names(list_time_period)[ind_i]," Finished with Asset: ",names(final_data)[ind_temp]))
  }
...
mat_p_val<-matrix(NA,nrow=length(final_data),ncol=9) #take QRF, QR, GJR_GARCH, GARCH-X, CAVIAR, Hist and 3x base, CAV_ASY
mat_perf<-matrix(NA,nrow=length(final_data),ncol=9) #take QRF, QR, GJR_GARCH, GARCH-X, CAVIAR, Hist, CAV_ASY
colnames(mat_p_val)<-colnames(mat_perf)<-test_rhs
rownames(mat_p_val)<-rownames(mat_perf)<-names(final_data)
for(ind_k in 1:length(final_data))
{
  temp_pred<-final_data[[ind_k]]
  true_y<-unlist(temp_pred$True_Ret)
  loss_lhs<-get_loss(loss=tick,true_val=true_y,forecast=unlist(temp_pred[[test_lhs]]), tau=0.05)
  cols_to_keep<-which(names(temp_pred) %in% test_rhs)
  #Make sure that they appear in right order
  ...
  print(paste0(" Finished with Asset: ",names(final_data)[ind_k],", ",ind_k," of ", length(final_data)))
}

cpa_file_name<-paste0("../..//Cleaned_Data/Tests/4 Periods/CPA_tests_full_",test_lhs,"_vs_rest.RData")
...
load(paste0("../..//Cleaned_Data/Tests/4 Periods/CPA_tests_full_",test_lhs,"_vs_rest.RData"))
...
#test_lhs<-"GRF" #GRF_base=GRF is default, GRF=GRF-X is the version with covariates
load(paste0("../../Cleaned_Data/Tests/4 Periods/CPA_tests_full_",test_lhs,"_vs_rest.RData"))
...
for(i in 1:4)
{
  ...
  if(i==1)
  {
    pdf(paste0("../../Results/Results Section/4 Periods/CPA/P_val_",test_lhs,"_Period_",tp,"_",var_to_order,".pdf"),width=10,height=5)
  ...
  if(i==3)
  {
    ...
    {
      pdf(paste0("../../Results/Results Section/4 Periods/CPA/P_val_",test_lhs,"_Period_",tp,".",j,"_",var_to_order,".pdf"),width=10,height=5)
...
ifelse(test_lhs=="GRF",vars_to_compare<-c(3,4,9),vars_to_compare<-c(4,5,9))
...
list_var_imp_qr <- vector("list", length(final_data))
for (i in seq_along(final_data)){
  var_imp_qr <- final_data[[i]][[14]]
  list_var_imp_qr[[i]] <- var_imp_qr
}
names(list_var_imp_qr) <- names(final_data)
...
names(df_qr_imp_sum)<-names(final_data)

# GRF

list_var_imp_grf <- vector("list", length(final_data))
for (i in seq_along(final_data)){
  var_imp_grf <- do.call(rbind, lapply(final_data[[i]][[11]], t))
  rownames(var_imp_grf)<-names(final_data[[i]][[11]])
  list_var_imp_grf[[i]] <- var_imp_grf
}
names(list_var_imp_grf) <- names(final_data)
...
which(names(final_data)=="btc")
num_asset<-16 #13,30, 100, btc, eth, xrp
num_assets<-which(names(final_data) %in% c("ada","btc","eth"))
name_assets<-names(final_data)[num_assets]
...
{
  ind_k<-num_assets[i]
  name_asset<-names(final_data)[ind_k]
...
ggsave(paste0("../../Results/Results Section/4 Periods/GRF_var_imp_",paste0(name_assets,collapse="_"),".pdf"),width=24,height = 18,units = "in")
